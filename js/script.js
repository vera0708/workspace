...
const createDetailVacancy = ({
    id, logo, company, title, description, salary,
    format, type, experience, location, email,
}) => `
<article class="detail">
    <div class="detail__header">
        <img src="${API_URL}${logo}" alt="Логотип копмании ${company}" class="detail__logo">
...
const init = () => {
    ...
    window.addEventListener('resize', () => {
        if (vacanciesFilter.classList.contains('vacancies__filter-btn_active')) {
            // можно:  vacanciesFilter.style.height = `${vacanciesFilter.scrollHeight}px`;
            // или скрываем в процессе изменения ширины: 
            closeFilter(
                vacanciesFilterBtn,
                vacanciesFilter,
                'vacancies__filter-btn_active',
